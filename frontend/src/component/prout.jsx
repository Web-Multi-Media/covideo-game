class App extends React.Component {
  ...
  fetchGifs = () => {
    fetch(`${this.state.url}api_key=${this.state.apiKey}&q=${this.state.searchTerm}&limit=${this.state.searchLimit}`).then(res => res.json()).then(data => this.setState({gifs: data.data, searching: false, searched: true}));
  }
        ...
        <div className="container">
          ...
          }
          {
            !this.state.searching && this.state.searched
              ? <SearchOutput gifs={this.state.gifs}/>
              : null
          }
        </div>
      </main>
...
function SearchInput({handleSearchInput}) {
  return (<div className="gif-search-container" onChange={handleSearchInput}>
    <div>
